...
function UserEdit({ user }: UserViewProps) {
  // console.log("edit user => ", user);

  let phoneStyle =
    user.phone.split(",").length == 2 ? user.phone.split(",")[1] : user.phone;

  let setUserRole = SingleOptions.find((e) => e.value == user.role);
  ...
  let setUserCountryCode = setUserPhoneObj.countryCode || "+880";
  let setUserCountryName = setUserCountryObj?.countryName  || "";
  ...
  const { data, setData, post, processing, errors, setError } = useForm({
    ...
    email: user.email,
    phone: phoneStyle,
    country: setUserCountryName,
    status: user.status,
    role: user.role,
    countryCode: setUserCountryCode,
  });
  ...
  const [selectedMulti2, setselectedMulti2] = useState<any>(setUserRole);
            ...
            <Col xs={12}>
              <Card>
                <Card.Header className="align-items-center d-flex">
                  <h4 className="mb-0 card-title flex-grow-1">User Update</h4>
                </Card.Header>
...
UserEdit.layout = (page: any) => <Layout children={page} />;
